...
function requireAuthEmail(request: { auth?: { token?: { email?: string } }; }): string {
...
async function audit(societyId: string, entry: Record<string, unknown>): Promise<void> {
  await db
...
export const acceptInvite = onCall({ region: REGION }, async (request) => {
  ...
  const societyId: string = invite.societyId;
  const now = new Date().toISOString();
  const userRecord = await import('firebase-admin/auth').then((m) =>
    m.getAuth().getUser(uid).catch(() => null)
  );

  const batch = db.batch();
  batch.set(
    db.collection('societies').doc(societyId).collection('members').doc(uid),
    {
      id: uid,
      uid,
      societyId,
  ...
  const existingIds: string[] = platformSnap.exists
    ? (platformSnap.data()!.societyIds as string[]) || []
    : [];
  batch.set(
    platformRef,
    {
...
export const confirmPayment = onCall({ region: REGION }, async (request) => {
  ...
  batch.set(
    db.collection('societies').doc(String(societyId)).collection('payments').doc(paymentId),
    {
      id: paymentId,
...
export const castVote = onCall({ region: REGION }, async (request) => {
  ...
  batch.update(
    db.collection('societies').doc(String(societyId))
      .collection('elections').doc(String(electionId))
      .collection('nominations').doc(String(candidateId)),
...
async function rebuildAnalytics(societyId: string): Promise<void> {
  ...
  for (const d of visitors.docs) {
    ...
    if (created >= thirtyDaysAgo) recentVisitors += 1;
  }
  ...
  for (const d of bookings.docs) {
    ...
    if (created.startsWith(monthPrefix)) monthBookings += 1;
  }
...
const triggerOpts = { region: REGION, database: DB_ID, retry: false } as const;

function analyticsTrigger(collection: string, name: string) {
  return onDocumentWritten(
    { ...triggerOpts, document: `societies/{societyId}/${collection}/{docId}` },
    async (event) => {
